...
export default function AppDashboard() {
  ...
  const [logs, setLogs] = useState<LogItem[]>([
    { id: 1, text: "REVENANT Core Initialized...",    type: "info"    },
    { id: 2, text: "Moorcheh Neural Sync: COMPLETE.", type: "success" },
    { id: 3, text: "NanoClaw sandbox: READY.",        type: "success" },
  ]);
  const [memory, setMemory] = useState<MemoryItem[]>([
    { id: 1, title: "Identity Fragment",  excerpt: "Anna Protocol v1.2.0 loaded.",               cluster: "Core"  },
    { id: 2, title: "Instruction Set",   excerpt: "Autonomous developer clone logic active.",    cluster: "Logic" },
  ]);
  ...
  const addMemory = useCallback((title: string, excerpt: string, cluster: string) => {
    setMemory(p => [{ id: Date.now() + Math.random(), title, excerpt, cluster }, ...p]);
  }, []);
  ...
  const startConversation = useCallback(async () => {
    try {
      addLog("Connecting to Tavus...", "info");
      const res  = await fetch("/api/tavus", { method: "POST" });
      const data = await res.json();
      if (res.status === 402) { setTavusError("402: Credits required"); addLog("Connection needs more credits.", "error"); return; }
      if (data.conversation_url) {
        setTavusUrl(data.conversation_url);
        setConversationId(data.conversation_id);
        addLog(`Session live: ${data.conversation_id?.slice(0,8)}`, "success");
      } else {
        setTavusError(data.error || "Unknown error");
        addLog(`Connection failed: ${data.error}`, "error");
      }
    } catch (e: any) { setTavusError(e.message); addLog(`Error: ${e.message}`, "error"); }
  }, [addLog]);
  ...
  const handleSend = useCallback(async () => {
    ...
    if (!q || isWorking) return;
    setChatInput("");

    const gh = q.match(GITHUB);
    if (gh) {
      const [,owner,repo] = gh;
      setSidebarOpen(true); setIsWorking(true);
      addLog(`Scouting github.com/${owner}/${repo}`, "info");
      try {
        const res  = await fetch(`https://api.github.com/repos/${owner}/${repo}/contents`).then(r => r.json());
        const tree = Array.isArray(res) ? res.map((f:any) => `${f.type==='dir'?'📂':'📄'} ${f.name}`).join("\n") : "";
        addLog("Repository indexed successfully.", "success");
        addMemory("Repo Indexed", `${owner}/${repo} synced.`, "Scout");
        if (conversationId) {
          fetch("/api/tavus/inject-context", {
            method:"POST", headers:{"Content-Type":"application/json"},
            body: JSON.stringify({ conversation_id: conversationId, context: `Repo ${owner}/${repo}:\n${tree}` }),
          }).catch(()=>{});
        }
      } catch(e:any){ addLog(`Scout error: ${e.message}`, "error"); }
      finally { setIsWorking(false); }
      return;
    }

    setSidebarOpen(true); setIsWorking(true);
    addLog(`Sending: ${q.slice(0,60)}...`, "info");
    setMessages(p => [...p, {role:"user",content:q}]);
    try {
      const res  = await fetch("/api/openai/v1/chat/completions", { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify({ messages:[...messages,{role:"user",content:q}] }) });
      const data = await res.json();
      if (data.choices?.[0]) {
        const rx = data.choices[0].message.content;
        addLog(`Anna: ${rx.slice(0,80)}...`, "success");
        setMessages(p => [...p, {role:"assistant",content:rx}]);
        if (data.context_metadata) addMemory("Knowledge Retrieved", "Semantic sync active.", "Recall");
      }
    } catch(e:any){ addLog(`Request failed: ${e.message}`, "error"); }
    finally { setIsWorking(false); }
  }, [chatInput, isWorking, conversationId, messages, addLog, addMemory]);

  const savePreference = useCallback(() => {
    if (!prefInput.trim()) return;
    addMemory("Preference Rule", prefInput, "Prefs");
    ...
    setPrefInput(""); setShowPrefs(false);
    addLog("Preference saved to Moorcheh.", "success");
      ...
      <header style={{ flexShrink:0, height:72, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 32px", borderBottom:`1px solid ${C.border}`, background:"rgba(12,9,5,0.9)", backdropFilter:"blur(24px)", position:"relative", zIndex:50 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:20 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            ...
            <span style={{ fontSize:10, color:C.muted, fontWeight:700, letterSpacing:"0.2em", textTransform:"uppercase" }}>Anna v1.2 — Active</span>
          </div>
        </div>

        {/* Right */}
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          <button onClick={() => setShowPrefs(!showPrefs)} title="Preferences" style={{ width:40, height:40, display:"flex", alignItems:"center", justifyContent:"center", borderRadius:10, border:`1px solid ${showPrefs ? C.borderGold : C.border}`, background: showPrefs ? "rgba(217,119,6,0.1)" : "transparent", color: showPrefs ? C.gold : C.muted, cursor:"pointer", transition:"all 0.2s" }}>
            <Settings style={{ width:16, height:16 }} />
          </button>
          <button onClick={() => setSidebarOpen(o => !o)} title="Toggle logs" style={{ width:40, height:40, display:"flex", alignItems:"center", justifyContent:"center", borderRadius:10, border:`1px solid ${sidebarOpen ? C.borderGold : C.border}`, background: sidebarOpen ? "rgba(217,119,6,0.1)" : "transparent", color: sidebarOpen ? C.gold : C.muted, cursor:"pointer", transition:"all 0.2s" }}>
        ...
        <div style={{ flex:1, display:"flex", flexDirection:"column", gap:12, minWidth:0 }}>
          ...
          <div style={{ flex:1, borderRadius:24, border:`1px solid ${C.border}`, background:"rgba(0,0,0,0.4)", overflow:"hidden", position:"relative", minHeight:0 }}>
            {tavusUrl ? (
              ...
              <div style={{ position:"absolute", inset:0, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:20 }}>
                ...
                <p style={{ fontSize:10, fontWeight:700, letterSpacing:"0.5em", textTransform:"uppercase", color:C.muted }}>Establishing Session</p>
                ...
                <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:20}} style={{ position:"absolute", bottom:20, left:20, right:20, padding:"28px", borderRadius:20, background:"rgba(12,9,5,0.95)", backdropFilter:"blur(32px)", border:`1px solid ${C.borderGold}`, zIndex:60 }}>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                      ...
                      <span style={{ fontFamily:"var(--font-playfair,Georgia)", fontStyle:"italic", fontSize:18, color:C.text }}>Synergy Rules</span>
                    </div>
                    <button onClick={() => setShowPrefs(false)} style={{ background:"transparent", border:"none", color:C.muted, cursor:"pointer" }}><X style={{width:16,height:16}} /></button>
                  </div>
                  <div style={{ display:"flex", gap:12 }}>
                    <input value={prefInput} onChange={e=>setPrefInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&savePreference()} placeholder="Define a behavioral rule for Anna..." style={{ flex:1, padding:"12px 16px", borderRadius:12, border:`1px solid ${C.border}`, background:"rgba(255,255,255,0.03)", color:C.text, fontSize:14, outline:"none" }} />
          ...
          <div style={{ flexShrink:0, display:"flex", alignItems:"center", gap:12, padding:"14px 20px", borderRadius:18, border:`1px solid ${C.border}`, background:"rgba(255,255,255,0.02)", backdropFilter:"blur(16px)" }}>
            ...
            <input
              ...
              placeholder="Ask Anna something, or paste a GitHub URL to scout a repo..."
            ...
            <div style={{ display:"flex", gap:8, flexShrink:0 }}>
              ...
              <button onClick={()=>setIsListening(o=>!o)} style={{ width:40, height:40, display:"flex", alignItems:"center", justifyContent:"center", borderRadius:10, border:`1px solid ${isListening ? "rgba(239,68,68,0.4)" : C.border}`, background: isListening ? "rgba(239,68,68,0.1)" : "transparent", color: isListening ? "#ef4444" : C.muted, cursor:"pointer", transition:"all 0.2s" }}>
              ...
              <div style={{ width:380, height:"100%", display:"flex", flexDirection:"column", gap:8 }}>
                ...
                <div style={{ flexShrink:0, display:"flex", gap:4, padding:4, borderRadius:14, border:`1px solid ${C.border}`, background:"rgba(255,255,255,0.015)" }}>
                  {(["terminal","memory"] as SidebarTab[]).map(tab => (
                    <button key={tab} onClick={()=>setSidebarTab(tab)} style={{ flex:1, padding:"10px 0", borderRadius:10, border:`1px solid ${sidebarTab===tab ? C.borderGold : "transparent"}`, background: sidebarTab===tab ? "rgba(217,119,6,0.1)" : "transparent", color: sidebarTab===tab ? C.gold : C.muted, fontWeight:700, fontSize:10, letterSpacing:"0.15em", textTransform:"uppercase", cursor:"pointer", transition:"all 0.2s" }}>
                      {tab === "terminal" ? <><Terminal style={{width:13,height:13,display:"inline",marginRight:6,verticalAlign:"middle"}} />Execution</> : <><Database style={{width:13,height:13,display:"inline",marginRight:6,verticalAlign:"middle"}} />Recall</>}
                    </button>
                  ))}
                </div>
                ...
                <div style={{ flex:1, display:"flex", flexDirection:"column", borderRadius:20, border:`1px solid ${C.border}`, background:"rgba(0,0,0,0.3)", overflow:"hidden", minHeight:0 }}>
                  ...
                  <div style={{ flexShrink:0, padding:"16px 20px", borderBottom:`1px solid ${C.border}`, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                    <span style={{ fontFamily:"var(--font-playfair,Georgia)", fontStyle:"italic", fontSize:16, color:C.text }}>{sidebarTab==="terminal" ? "Protocol Logs" : "Memory Recall"}</span>
                  ...
                  <div style={{ flex:1, overflowY:"auto", padding:"16px 20px" }}>
                    {sidebarTab === "terminal" ? (
                      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                        {logs.map(l => (
                          <div key={l.id} style={{ display:"flex", gap:10, fontSize:12, fontFamily:"monospace", color: l.type==="error" ? "#f87171" : l.type==="success" ? C.gold : C.muted, lineHeight:1.5 }}>
                      ...
                      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                        {memory.map(m => (
                          <div key={m.id} style={{ padding:"16px", borderRadius:14, border:`1px solid ${C.border}`, background:C.surface }}>
                            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8 }}>
                              <span style={{ fontSize:9, fontWeight:700, letterSpacing:"0.2em", textTransform:"uppercase", color:C.gold }}>{m.cluster}</span>
                              <ChevronRight style={{width:12,height:12,color:C.muted}} />
                            </div>
                            <p style={{ fontSize:12, fontWeight:700, color:C.text, marginBottom:4 }}>{m.title}</p>
                            <p style={{ fontSize:11, color:C.muted, fontFamily:"monospace", lineHeight:1.5 }}>{m.excerpt}</p>
                          </div>
                        ))}
                        <div ref={memoryEnd} />
                      </div>
                  ...
                  <div style={{ flexShrink:0, padding:"10px 20px", borderTop:`1px solid ${C.border}`, display:"flex", alignItems:"center", gap:6 }}>
                    ...
                    <span style={{ fontSize:9, color:C.muted, fontWeight:700, letterSpacing:"0.2em", textTransform:"uppercase" }}>Internal Link Active</span>
                  </div>
                </div>
              </div>
            </motion.div>
