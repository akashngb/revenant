...
function StatusLine() {
  const [decisions, setDecisions] = useState("0.0");

  useEffect(() => {
    const interval = setInterval(() => {
      setDecisions((Math.random() * 12 + 3).toFixed(1));
    }, 2000);
    return () => clearInterval(interval);
  }, []);
  ...
  return (
    <div className="flex items-center gap-2 text-[10px] tracking-widest text-muted-foreground uppercase font-mono">
      ...
      <span>live capture: {decisions}k decisions / day</span>
    </div>
  );
}
...
const TIERS: Tier[] = [
  {
    ...
    period: "/ forever",
    tag: null,
    description: "Community-grade memory. 5 integrations. No SLA.",
    features: [
      { text: "500 decisions / month", included: true },
      { text: "Slack + GitHub integrations", included: true },
      { text: "Shared memory pool", included: true },
      { text: "Basic AI recall", included: true },
      { text: "Priority capture", included: false },
  ...
  },
  {
    ...
    period: "/ month",
    tag: "RECOMMENDED",
    description: "Full context capture. All integrations. 99.97% uptime SLA.",
    features: [
      { text: "Unlimited decisions", included: true },
      { text: "All integrations (Slack, GitHub, Jira, Gmail)", included: true },
      { text: "Dedicated memory core", included: true },
      { text: "AI mentor — full context", included: true },
      { text: "Priority capture + recall", included: true },
      { text: "Dedicated support", included: false },
    ],
    cta: "START BUILDING",
    highlighted: true,
  },
  {
    ...
    period: "",
    tag: null,
    description: "Air-gapped. On-prem. Custom model fine-tuning. Full control.",
    features: [
      { text: "Unlimited everything", included: true },
      { text: "Custom integrations", included: true },
      { text: "On-premises deployment", included: true },
      { text: "Custom AI mentor fine-tuning", included: true },
...
export function PricingSection() {
  return (
    <section id="pricing" className="w-full px-6 py-20 lg:px-12">
      ...
      >
        <div className="flex flex-col gap-3">
          ...
          <p className="text-xs lg:text-sm font-mono text-muted-foreground leading-relaxed max-w-md">
            ...
            and access to the Revenent memory API.
          </p>
        </div>
        <StatusLine />
      </motion.div>
      ...
      >
        <span className="text-[10px] tracking-[0.2em] uppercase text-muted-foreground font-mono">
          {"* All plans billed annually. Cancel anytime. Your data, your memory."}
        </span>
        <div className="flex-1 border-t border-border" />
      </motion.div>
    </section>
